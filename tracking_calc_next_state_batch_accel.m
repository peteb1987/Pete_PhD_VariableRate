...
new_x1(bnz) = x1 + ((new_sdot(bnz).^2)./SF1(bnz)).*( aP(bnz).*sin(new_psi(bnz))+2*aT(bnz).*cos(new_psi(bnz))) - ((sdot^2)./SF1(bnz)).*( aP(bnz).*sin(psi)+2*aT(bnz).*cos(psi)) + aX1(bnz)*dt;
new_x2(bnz) = x2 + ((new_sdot(bnz).^2)./SF1(bnz)).*(-aP(bnz).*cos(new_psi(bnz))+2*aT(bnz).*sin(new_psi(bnz))) - ((sdot^2)./SF1(bnz)).*(-aP(bnz).*cos(psi)+2*aT(bnz).*sin(psi)) + aX2(bnz)*dt;

% aT is zero but aP isn't
new_x1(oaTz) = x1 + ((new_sdot(oaTz).^2)./aP(oaTz)).*( sin(new_psi(oaTz)) - sin(psi) ) + aX1(oaTz)*dt;
new_x2(oaTz) = x2 + ((new_sdot(oaTz).^2)./aP(oaTz)).*(-cos(new_psi(oaTz)) + cos(psi) ) + aX2(oaTz)*dt;

% ap is zero but aT isn't
new_x1(oaPz) = x1 + 0.5*dt*cos(psi).*new_sdot(oaPz) + aX1(oaPz)*dt;
new_x2(oaPz) = x2 + 0.5*dt*sin(psi).*new_sdot(oaPz) + aX2(oaPz)*dt;

% Both aT and aP are zero
new_x1(bz) = x1 + ( sdot*dt.*cos(psi) ) + aX1(bz)*dt;
new_x2(bz) = x2 + ( sdot*dt.*sin(psi) ) + aX2(bz)*dt;
