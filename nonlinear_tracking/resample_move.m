function [ pt ] = resample_move( flags, params, k, pt, times, observ, type )
...
switch type
    ...
    case 1
        ...
        [w_replace, ppsl_prob, rev_ppsl_prob] = tracking_acceleration_proposal(flags, params, last_x, last_tau, last_w, times(1:k), observ(:,1:k));
        
        % Calculate likelihoods
        old_lhood = pt.lhood(start_idx:end);
        [new_lhood, new_intx] = interpolate_state(flags, params, last_tau, last_x, w_reaplce, times(start_idx:k), observs(start_idx:k));
    ...
    case 2
        ...
        [w_replace, ppsl_w_prob, ~] = tracking_acceleration_proposal(flags, params, x_replace, tau_replace, last_w, times(1:k), observ(:,1:k));
        
        % Calculate reverse proposal probabilities
        rev_ppsl_tau_prob = log(normpdf(last_tau, tau_replace, params.ppsl_move_time_sd));
        [~, ~, rev_ppsl_w_prob] = tracking_acceleration_proposal(flags, params, last_x, last_tau, last_w, times(1:k), observ(:,1:k));
        ...
        [new_lhood, ppsl_intx] = tracking_calc_likelihood(flags, params, x_replace, tau_replace, w_replace, times(1:k), observ(:,1:k));
        if replace_start_idx>last_start_idx
            [gap_lhood(1:replace_start_idx-1), ppsl_intx(:,1:replace_start_idx-1)] = tracking_calc_likelihood(flags, params, penult_x, penult_tau, penult_w, times(1:replace_start_idx-1), observ(:,1:replace_start_idx-1), last_start_idx);
