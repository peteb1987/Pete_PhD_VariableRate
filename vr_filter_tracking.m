                ...
                if (ch>1)%&&((t-last_tau)>(params.dt*20))%%&&(rand<0.05)
                    ...
                    if (pts_Ns(jj)>1)&&(rand<0.5)
                        ...
                        if (last_tau+params.dt<t)&&(rand<0.5)
                            ...
                            [new_lhood_part1, ppsl_intx] = tracking_calc_likelihood(flags, params, last_x, last_tau, w1, times(1:stop_idx), observ(:,1:stop_idx));
                            [new_lhood_part2, ppsl_intx] = tracking_calc_likelihood(flags, params, extra_x, extra_tau, w2, times(1:kk), observ(:,1:kk));
                            ...
                            % MH acceptance
                            acc_prob = (sum(new_lhood)+new_accel_prob+new_trans_prob)-(sum(old_lhood)+old_accel_prob+old_trans_prob)-(ppsl_prob);
                            if log(rand)<acc_prob
                                pts_Ns(jj) = pts_Ns(jj)+1;
                        ...
                        elseif (pts_Ns(jj)>1)
                            ...
                            rev_ppsl_prob = rev_ppsl_prob_part1 + rev_ppsl_prob_part2;
                            old_lhood = sum(pts_lhood(jj,start_idx:end));
                            ...
                            % MH acceptance
                            acc_prob = (sum(new_lhood)+new_accel_prob+new_trans_prob)-(sum(old_lhood)+old_accel_prob+old_trans_prob)+(rev_ppsl_prob);
                            if log(rand)<acc_prob
                                pts_Ns(jj) = pts_Ns(jj)-1;
