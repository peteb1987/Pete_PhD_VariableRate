...
for test = 1:num_test
    ...
    comb_kiti.mospa(test) = kiti.mospa;
    comb_kiti.unique_sequences(test) = kiti.unique_sequences;
    comb_kiti.unique_times(test) = kiti.unique_times;
    comb_kiti.MMSE_value_RMSE(test) = kiti.mean_rmse.value;
    comb_kiti.MMSE_trend_RMSE(test) = kiti.mean_rmse.trend;
    comb_kiti.MAP_value_RMSE(test) = kiti.MAP_rmse.value;
    comb_kiti.MAP_trend_RMSE(test) = kiti.MAP_rmse.trend;
    
    comb_VRPS.mospa(test) = VRPS.mospa;
    comb_VRPS.unique_sequences(test) = VRPS.unique_sequences;
    comb_VRPS.unique_times(test) = VRPS.unique_times;
    comb_VRPS.MMSE_value_RMSE(test) = VRPS.mean_rmse.value;
    comb_VRPS.MMSE_trend_RMSE(test) = VRPS.mean_rmse.trend;
    comb_VRPS.MAP_value_RMSE(test) = VRPS.MAP_rmse.value;
    comb_VRPS.MAP_trend_RMSE(test) = VRPS.MAP_rmse.trend;
end
...
fprintf(1, 'Filter:          %f  %f  %f  %f  %f  %f  %f.\n', 0, 0, 0, mean(comb_filt.MMSE_value_RMSE), mean(comb_filt.MMSE_trend_RMSE), mean(comb_filt.MAP_value_RMSE), mean(comb_filt.MAP_trend_RMSE));
